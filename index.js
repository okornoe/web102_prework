...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        gameCard.innerHTML = `<img class="game-img" src="${games[i].img}" alt="image" />
                              <h3>${games[i].name}</h3>
...
const totalBackers = GAMES_JSON.reduce((acc, bck) => {
    //console.log(acc + bck.backers);
    return acc + bck.backers;
...
const amountRaised = GAMES_JSON.reduce((acc, amt) =>{
    return acc + amt.pledged;
}, 0).toLocaleString('en-US');
...
let totalamt = 0;
let numOfFundedGames = 0;
let numOfUnFundedGames = 0;
const amtRaisedForFundedGames = GAMES_JSON.reduce((acc, games) =>{
    if(games.pledged >= games.goal) {
        totalamt = acc + games.pledged;
        numOfFundedGames++;
    }
    return totalamt;
}, 0).toLocaleString('en-US');
